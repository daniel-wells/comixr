...
#   shape=c(0.125),
#   component.type=c("common","specific"))
# output.vb <- fit.model(input, initial.parameters.vb, algorithm = "VB",max.iterations = 20)
...
```{r}
output.test.basic.vb.1 <- fit.model(test.data.basic[,.(vals,seg)], initial.parameters.basic.vb.1, max.iterations = 60, algorithm = "VB", quiet = TRUE)
```
...
This is also a good case to demonstrate the failure of this algorithm. Depending on the inital means the method can get stuck in a local maxima. E.g. change the mean 0.6 to 0.5.
...
output.test.basic.vb.2 <- fit.model(test.data.basic[,.(vals,seg)], initial.parameters.basic.vb.2, max.iterations = 60, algorithm = "VB", quiet = TRUE)
...
output.test.basic.vb.3 <- fit.model(test.data.basic[,.(vals,seg)], initial.parameters.basic.vb.3, max.iterations = 60, algorithm = "VB")
```
...
output.realistic <- fit.model(test.data.realistic[,.(vals,seg)], initial.parameters.realistic, quiet = TRUE)
...
# 
# output.realistic.vb <- fit.model(test.data.realistic.vb[,.(vals,seg)], initial.parameters.realistic.vb, max.iterations=100, algorithm="VB")
# 
